...
for (i in 1:k) {
  ...
  # Fit model
  lm_tower_obj <- lm_tower(train, yName = "income_num")
  #lm_prefill_model <- lm_prefill(train_complete, "income_num", impute_method = "complete", use_dummies = FALSE)
  
  #Predict Prepare
  validation_pred <- validation_complete
  validation_pred$income_num <- NULL
  
  # Predict
  predictions <- predict(lm_tower_obj, newdata = validation_pred) #using the updated validation set
