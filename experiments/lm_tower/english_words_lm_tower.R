...
#mpg <- read.csv(url, header = FALSE, col.names = colnames,na.strings = "?", strip.white = TRUE, sep = "")
english <- read.csv("English.csv", na.strings=c("", "NA", "?"))
...
for (i in 1:k) {
  ...
  train <- english[fold_ids != i, ]
  validation <- english[fold_ids == i, ]
  ...
  # Fit model
  lm_tower_obj <- lm_tower(train, yName = "vocab")
  ...
  true_values <- validation_complete$vocab
